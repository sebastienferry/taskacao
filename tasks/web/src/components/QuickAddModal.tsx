...
export const QuickAddModal: React.FC = () => {
  ...
  const [source, setSource] = useState<TaskSource>(
    (settings.issueTracker === 'github' || settings.issueTracker === 'linear') ? settings.issueTracker : 'local'
  )
  ...
  useEffect(() => {
    if (isQuickAddOpen) {
      ...
      setPriority('medium')
      setSource((settings.issueTracker === 'github' || settings.issueTracker === 'linear') ? settings.issueTracker : 'local')
      setLabels(['New'])
      setLabelInput('')
      setTimeout(() => {
  ...
  }, [isQuickAddOpen, quickAddInitialStatus, settings.issueTracker])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {/* Source Tracker Selector (Linear / GitHub / Local) */}
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <span>Linear ({settings.linearTeam || 'FRE'})</span>
              </button>
